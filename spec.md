...
- **No silent data loss.** The lock-escape snapshot takes one consistent copy through the backup API (WAL content folded in; since 1.23). A snapshot torn mid-checkpoint can silently read stale data; the backup API cannot tear, and only the leash fallback (a writer holding the lock past `SNAPSHOT_LEASH`) retains the old copy-the-sidecars shape. The temp files are cleaned up on `close()` and on context manager exit.
